...
%% Selecting features (colour, roundness) for training

x1 = [hsv_value_A1 hsv_value_A2 hsv_value_A3 hsv_value_A4 hsv_value_A5 hsv_value_A6 hsv_value_A7 hsv_value_A8 hsv_value_A9   hsv_value_P1 hsv_value_P2 hsv_value_P3 hsv_value_P4];   % color
x2 = [metric_A1 metric_A2 metric_A3 metric_A4 metric_A5 metric_A6 metric_A7 metric_A8 metric_A9    metric_P1 metric_P2 metric_P3 metric_P4];     % roundness

% estimated features matrix 
P = [x1; x2];   

% Desired output vector T 
...
% Initial outputs and errors with random parameters
e = 0;
...
while (iter < maxIter)   % executes while the total error is not 0
    iter = iter + 1;
    
        % go through all 13 training examples
        for n = 1:13
            
            % current inputs and desired output
            x1n = x1(n);
            x2n = x2(n);
            dn  = T(n);     % desired output d(n)
